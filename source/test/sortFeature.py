# 基于word 
# python sortFeature.py train.words2 > log
...
def get_vocab_1gram(file_path,is_word = True):
    '''
...
def TextToFeature1gram(text,rev_vocab):
    fea = np.zeros((len(rev_vocab)))  # 行向量
    for char in text:
        try:
            fea[rev_vocab[char]]=1
        except:
            pass
    return list(fea) # 输出矩阵 len(text) * len(rev_vocab)


def createDataSet_1gram(file_path,is_word=True):
    vocab,rev_vocab = get_vocab_1gram(file_path)
...
def main():
    dataSet, labels = createDataSet_1gram(sys.argv[1])
